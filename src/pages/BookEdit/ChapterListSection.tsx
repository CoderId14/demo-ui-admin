...
import { Divider, Space, Skeleton, List, Typography, Alert, Popconfirm, Button } from "antd";
...
function ChapterListSection({ bookId }: Props) {
  ...
  return (
    <>
          ...
          <Space align='baseline'>
            {latestChapter && (
              <>
                ...
                <Paragraph>
                  {formatDistance(
                    new Date(latestChapter.modifiedDate),
                    new Date(),
                    {
                      addSuffix: true,
                    },
      ...
      <Divider></Divider>
      <Button type="primary" onClick={()=>navigate(AppConst.CHAPTER_ADMIN_ADD_URL + bookId)}>Add New Chapter</Button>
      <List
        bordered
        dataSource={chapterData}
        renderItem={(item) => (
          <List.Item>
            <Typography.Text>Chapter {item?.chapterNumber}: {item?.title}</Typography.Text>
            <Space size='middle'>
